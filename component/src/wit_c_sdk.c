...
int32_t WitWriteReg(uint32_t uiReg, uint16_t usData)
{
    ...
    switch(s_uiProtoclo)
    {
        ...
        case WIT_PROTOCOL_I2C:
            ...
            ucBuff[1] = usData >> 8;
            if(p_WitI2cWriteFunc(s_ucAddr << 1, uiReg, ucBuff, 2) != 1)printf("i2c write fail\r\n");
            break;
		default:
			return WIT_HAL_INVAL;
    }
...
int32_t WitReadReg(uint32_t uiReg, uint32_t uiReadNum)
{
    ...
    switch(s_uiProtoclo)
    {
        ...
        case WIT_PROTOCOL_I2C:
            ...
            if(p_WitI2cReadFunc(s_ucAddr << 1, uiReg, ucBuff, uiReadNum * 2) != 1)printf("i2c read fail\r\n");
            break;
		default:
			return WIT_HAL_INVAL;
    }
...
{
	/*
	if(WitWriteReg(CALSW, CALACC) != WIT_HAL_OK) return WIT_HAL_ERROR;
	if(WitWriteReg(SAVE, SAVE_PARAM) != WIT_HAL_OK) return WIT_HAL_ERROR;
	*/
	if(WitWriteReg(CALSW, CALACCL) != WIT_HAL_OK) return WIT_HAL_ERROR;
    return WIT_HAL_OK;
}
...
{
	if(!CheckRange(uiRate,RRATE_02HZ,RRATE_100HZ))
	{
		return WIT_HAL_INVAL;
	}
